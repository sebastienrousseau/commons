                ...
                #[cfg(target_os = "macos")]
                {
                        ...
                        || std::env::var("GITHUB_ACTIONS").is_ok()
                    {
                        ...
                        #[cfg(not(any(test, miri)))]
                        {
                            ...
                            let subsystem =
                                CString::new("com.euxis.logging").unwrap();
                            let category =
...
mod tests {
    ...
    fn test_from_config_file_destination() {
        ...
        let config = super::super::log_config::LoggingConfig {
            logging_destinations: vec![
                super::super::log_config::LoggingDestination::File(
                    log_path,
                ),
            ],
        ...
        assert!(
            matches!(sink, PlatformSink::File(_)),
            "Expected File sink"
        );
    }
    ...
    fn test_from_config_stdout_destination() {
        let config = super::super::log_config::LoggingConfig {
            logging_destinations: vec![
                super::super::log_config::LoggingDestination::Stdout,
            ],
            ..super::super::log_config::LoggingConfig::default()
        };
        let sink = PlatformSink::from_config(&config);
        assert!(
            matches!(sink, PlatformSink::Stdout),
            "Expected Stdout sink"
        );
    }
    ...
    fn test_from_config_network_only_falls_through() {
        let config = super::super::log_config::LoggingConfig {
            logging_destinations: vec![
                super::super::log_config::LoggingDestination::Network(
    ...
    fn test_journald_emit_all_levels() {
        #[cfg(unix)]
        {
            ...
            let levels = [
                "ERROR", "FATAL", "CRITICAL", "WARN", "INFO",
                "DEBUG", "TRACE", "VERBOSE", "UNKNOWN_LEVEL",
            ];
            for level in &levels {
                let (sock, _peer) = UnixDatagram::pair().unwrap();
                let mut sink =
                    PlatformSink::Journald(Some(sock));
